...
/// TEMP: is `argmax(1)` + id readback over `[n, 128256]` logits the serving linearizer?
#[test]
#[ignore = "microbench: run manually on a GPU backend"]
fn bench_argmax_readback() {
    let device: Device = Default::default();
    let vocab = 128256usize;
    for n in [1usize, 4, 16] {
        let logits =
            Tensor::<2>::random([n, vocab], burn::tensor::Distribution::Default, &device);
        // warmup
        let _ = logits.clone().argmax(1).into_data();
        let reps = 10;
        let t = std::time::Instant::now();
        for r in 0..reps {
            let l = logits.clone() + (r as f32); // defeat caching
            let _ = l.argmax(1).into_data();
        }
        println!("n={n:>2} argmax+readback: {:>8.2} ms", t.elapsed().as_secs_f64() * 1000.0 / reps as f64);
    }
}

/// TEMP: does the Gumbel top-p path reproduce the measured sample times (92 ms @ n=1,
/// ~1000 ms @ n=16) in isolation? Times the exact op sequence of `LlamaSampler::sample`'s
/// stochastic branch at serving shapes.
#[test]
#[ignore = "microbench: run manually on a GPU backend"]
fn bench_gumbel_top_p_path() {
    let device: Device = Default::default();
    let vocab = 128256usize;
    for n in [1usize, 4, 16] {
        let logits =
            Tensor::<2>::random([n, vocab], burn::tensor::Distribution::Default, &device);
        let run = |l: Tensor<2>| {
            let probs = burn::tensor::activation::softmax(l / 0.7, 1);
            let (sorted, idx) = probs.sort_descending_with_indices(1);
            let exclusive = sorted.clone().cumsum(1) - sorted.clone();
            let dropped = exclusive.greater_equal_elem(0.9);
            let masked = sorted.log().mask_fill(dropped, f32::NEG_INFINITY);
            let uniform = Tensor::<2>::random(
                [n, vocab],
                burn::tensor::Distribution::Uniform(1e-7, 1.0),
                &device,
            );
            let gumbel = uniform.log().neg().log().neg();
            let winner = (masked + gumbel).argmax(1);
            let _ = idx.gather(1, winner).into_data();
        };
        run(logits.clone()); // warmup/autotune
        let reps = 5;
        let t = std::time::Instant::now();
        for r in 0..reps {
            run(logits.clone() + (r as f32));
        }
        println!("n={n:>2} gumbel top-p: {:>8.1} ms", t.elapsed().as_secs_f64() * 1000.0 / reps as f64);
    }
}
