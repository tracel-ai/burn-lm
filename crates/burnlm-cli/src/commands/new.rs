...
const CRATE_DIR: &'static str = "./crates";
const TEMPLATE_DIR_NAME: &'static str = "burnlm-inference-template";
const REGISTRY_DIR_NAME: &'static str = "burnlm-registry";
...
pub(crate) fn handle(args: &clap::ArgMatches) -> anyhow::Result<()> {
    ...
    let crate_name = args.get_one::<String>("name").expect("The name argument should be set.");
    ...
    if std::fs::metadata(&crate_path).map(|m| m.is_dir()).unwrap_or(false) {
    ...
    replacements.insert("burnlm-inference-template".to_string(), crate_fullname.clone());
    ...
    let mut replacements = std::collections::HashMap::new();
    replacements.insert("burnlm_inference_template".to_string(), crate_namespace.clone());
    ...
    let registry_lib = std::path::Path::new(CRATE_DIR).join(REGISTRY_DIR_NAME).join("src").join("lib.rs");
    ...
    let registry_cargo_path = std::path::Path::new(CRATE_DIR).join(REGISTRY_DIR_NAME).join("Cargo.toml");
    let dependency = format!("{crate_fullname} = {{ path = \"../{crate_fullname}\" }}");
    let mut registry_cargo_file = std::fs::OpenOptions::new().append(true).open(registry_cargo_path)?;
...
pub fn add_inference_server_registration<P: AsRef<std::path::Path>>(
    ...
    let original = std::fs::read_to_string(file_path).expect(
        &format!("should be able to read file '{}'", file_path.to_str().unwrap()));
    ...
    let start_index = original.find(start_marker).ok_or_else(|| anyhow::format_err!("registry file should contain the inference_server_registry attribute"))?;
...
"#;
    ...
    fn test_add_inference_server_registration(
        ...
        if expect_success {
            ...
            let final_contents = std::fs::read_to_string(path)
                .expect("should read updated lib content");
            let inserted_text = format!(
                "server(\n        crate_namespace = \"{crate_namespace}\",\n        server_type = \"{server_type}\",\n    ),"
            );
            assert!(final_contents.contains(&inserted_text), "Updated file should contain the new server(...) entry.");
        } else {
